...
async function enrichArticle(item) {
  ...
  const prompt = [
    ...
    "titleKo는 카드에서 기사 성격을 즉시 알 수 있는 자연스러운 한국어 제목으로, reportBullet은 날짜로 시작하는 보고서 소제목으로 각각 작성하라. titleKo·reportBullet 모두 원문 제목의 기계적 번역이나 '혼란 발생', '우려 증가', '주목 필요' 같은 추상적 표현만으로 끝내지 말라.",
    ...
    "불법 드론 제조시설 적발 기사 제목은 '이라크 국가안보국(NSS), Baghdad 내 불법 드론 제조시설 적발'처럼 기관·장소·사안을 구체적으로 작성한다. NSS는 '국가안보국(NSS)'으로 통일하며, '드론 제조 음모 차단' 같은 추상적·과장된 제목은 사용하지 않는다. summaryKo에는 용의자 수, 드론 기체·부품 등 압수 규모, 최종 조립·운용 배치 전 적발 여부, 원문에 명시된 추가 수사 범위를 포함한다.",
    "지역 무장세력 연계·자재 조달망 등은 원문에 확대 수사 사실이 있을 때만 '연계 및 조달망 확대 조사 중'처럼 수사 단계로 표기하고, 실제 연계 사실로 단정하지 않는다. 이 유형의 reportBullet은 'M.D, 국가안보국(NSS), Baghdad 내 불법 드론 제조시설 적발' 형식을 우선하며, reportSubBullets에는 체포·압수 결과와 추가 수사 범위를 각각 한 줄로 정리한다.",
